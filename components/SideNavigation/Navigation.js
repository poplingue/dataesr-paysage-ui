        ...
        >
          {!mobile && (
            ...
            >
              ...
              >
                <Text
                  className={`${sideMode === "on" ? "hidden" : ""} marianne`}
                  as="span"
                  size="md"
                >
                  Navigation
          ...
          <ul
            className={`${styles.SideNavContent} ${sideMode === "off" &&
              styles.Active}`}
          >
            {items.map(section => {
              ...
              if (component && content.length > 0) {
                return (
                  <SideMenuItem
                    key={title}
                    className={sideMode === "on" ? "hidden" : ""}
                    title={title}
                    expandedDefault
                  >
                    {content.map(subSection => {
                      const { title: subSectionTitle } = subSection;

                      return (
                        <SideMenuLink
                          className={`${sideMode === "on" &&
                            "hidden"} marianne`}
                          onClick={e =>
                            goToSection(e, sectionUniqueId(subSectionTitle))
                          }
                          href="/"
                          key={subSectionTitle}
                        >
              ...
              return (
                <SideMenuLink
                  className={sideMode === "on" ? "hidden" : ""}
                  onClick={e =>
                    goToSection(e, sectionUniqueId(title, content.length))
                  }
                  href="/"
