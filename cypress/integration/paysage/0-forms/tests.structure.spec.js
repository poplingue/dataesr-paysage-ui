...
context('Structure form page', () => {
    ...
    it('should switch toggle value', () => {
        cy.get('[data-cy="tests/structure@etat#0/test#0"]').click();
        cy.get('[data-cy="tests/structure@etat#0/test#0"]')
            .find('input')
            .should('be.checked');
    ...
    it('should render new Toggle', () => {
        ...
        cy.get('[data-cy="tests/structure@etat#1/test#0"]').should('exist');
    ...
    it('should clean new Toggle', () => {
        ...
        cy.get('[data-cy="tests/structure@etat#1/test#0"]').should('not.exist');
    });
    ...
    it('should select catégorie 2 Type value', () => {
        cy.get('[data-testid="tests/structure@categories#0/type#0"]')
            .find('select')
            .select('catégorie 2');
        cy.get('[data-testid="tests/structure@categories#0/type#0"]')
            .find('select')
    ...
    it('should add, change and delete Catégorie section', () => {
        cy.get('[data-testid="tests/structure@categories#0/type#0"]')
            ...
            .select('catégorie 1');
        cy.get('[data-testid="btn-add-categories"]').click();
        cy.get('[data-testid="tests/structure@categories#1/type#0"]')
            .find('select')
            .select('catégorie 2');
        cy.get('[data-testid="tests/structure@categories#1/startday#0"]')
            .find('select')
            .select('12');
        cy.get('[data-testid="tests/structure@categories#1/endmonth#0"]')
            .find('select')
            .select('12');
        cy.get('[data-testid="tests/structure@categories#1/endyear#0"]')
            .find('select')
            .select('2012');
        cy.get('[data-testid="btn-add-categories"]').click();
        cy.get('[data-testid="tests/structure@categories#2/endday#0"]')
            .find('select')
            .select('1');
        cy.get('[data-testid="tests/structure@categories#2/startmonth#0"]')
            .find('select')
            .select('1');
        cy.get('[data-testid="tests/structure@categories#2/endyear#0"]')
            .find('select')
            .select('2000');
        cy.get('[data-testid="btn-delete-categories#1"]').click({
            force: true,
        ...
        cy.wait(1000);
        cy.get('[data-testid="tests/structure@categories#1/endday#0"]')
            .find('select')
            .should('have.value', '1');
        cy.get('[data-testid="tests/structure@categories#1/startmonth#0"]')
            .find('select')
            .should('have.value', '1');
        cy.get('[data-testid="tests/structure@categories#1/endyear#0"]')
            .find('select')
    ...
    it('should select today date', () => {
        ...
        cy.get('[data-testid="tests/structure@categories#0/startday#0"]')
            .find('select')
            .should('have.value', now.getDate());
        cy.get('[data-testid="tests/structure@categories#0/startmonth#0"]')
            .find('select')
            .should('have.value', now.getMonth() + 1);
        cy.get('[data-testid="tests/structure@categories#0/startyear#0"]')
            .find('select')
    ...
    it('should select 1st january date', () => {
        ...
        cy.get('[data-testid="tests/structure@categories#0/endday#0"]')
            .find('select')
            .should('have.value', '1');
        cy.get('[data-testid="tests/structure@categories#0/endmonth#0"]')
            .find('select')
            .should('have.value', '1');
        cy.get('[data-testid="tests/structure@categories#0/endyear#0"]')
            .find('select')
            .should('have.value', now.getFullYear());
    });
});
